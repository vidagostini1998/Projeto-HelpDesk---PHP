...
<body>
    <header>
        ...
        <div class="px-3 py-2 border-bottom mb-3">
            <div class="text-end log">
                <a class="btn btn-primary" <?php echo $criar;?> href="?page=adc_user"><i class="fas fa-plus-circle"></i></a>
                <a class="btn btn-primary" href="?page=inicio_adm"><i class="fas fa-arrow-circle-left"></i></a>
            </div>
        </div>
    </header>

    <div class="container-fluid">
        <h2 style="padding-bottom:20px;">Usuarios</h2>
        ...
        <table class="table table-hover text-center" id="table1">
          ...
          include('./db.php');
            while($linha = mysqli_fetch_array($consulta_users)){
                ...
                if($linha['habilitado'] == 0){
                    $habilitado = "bg-danger";
                }
                else{
                    $habilitado="";
                }
              echo '<tr class="'.$habilitado.'"><td><a href="?page=alter_user&id='.$linha['id'].'" class="btn btn-primary btn-lg"'.$alterar.'>'.$linha['id'].'</a></td> ';
              echo '<td>'.$linha['nome'].'</td>';
              echo '<td>'.$linha['email'].'</td>';
              echo '<td>'.$linha['user'].'</td>';              
              if($linha['habilitado']==1){
                echo '<td> <a '.$excluir.' href="?page=exclui_user&id='.$linha['id'].'"><img src="./img/excluir.png"></a></td>';
              }
              else if($linha['habilitado'] ==0){
                echo '<td> <a '.$excluir.' href="?page=habilita_user&id='.$linha['id'].'"><img style="width:32px;height:32px;" src="./img/verificar.png"></a></td>';
              }
    ...
    <?php
include ('./Administracao/Usuarios/footer.php')
?>
</body>
