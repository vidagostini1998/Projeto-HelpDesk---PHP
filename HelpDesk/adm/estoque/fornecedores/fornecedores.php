    ...
    <header>
        ...
        <div class="px-3 py-2 border-bottom mb-3">
            <div class="text-end log">
                <a class="btn btn-primary" href="?page=adc_fornecedor"><i class="fas fa-plus-circle"></i></a>
        ...
        <div class="container-fluid">
          ...
          include('./db.php');
            while($linha = mysqli_fetch_array($consulta_fornecedor)){
              ...
              echo '<tr><td><a href="?page=alter_entrada&id='.$linha['id_fornecedor'].'" class="btn btn-success btn-lg"  >'.$linha['id_fornecedor'].'</a></td> ';
              echo '<td>'.$linha['nome_fornecedor'].'</td>';
              echo '<td>'.$linha['cnpj_cpf'].'</td>';
              echo '<td>'.$linha['endereco'].'</td>';
              echo '<td>'.$linha['tel'].'</td>';
              echo '<td>'.$linha['cel'].'</td>';
              echo '<td>'.$linha['obs'].'</td>';
              echo '<td>'.$linha['data_adc'].'</td></tr>';
            }
          ?>
                </tbody>
            </table>
