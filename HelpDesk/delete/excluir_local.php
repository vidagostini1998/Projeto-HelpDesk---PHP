...
<script>
    function excluir() {
        var resultado = confirm("Você realmente deseja excluir o local?");
        if (resultado == true) {
            document.getElementById("form1").submit();

        } else {
            window.location.href = "?page=consulta_local";
        }
    }
</script>
...
<div class="container-fluid">
    <?php
    while($linha=mysqli_fetch_array($consulta_alter_local)){ 
    if($linha['id_local']==$_GET['id']){
?>
    <form class="row g-3" id="form1" action="./HelpDesk/delete/pro_del_local.php" method="POST">
        <input type="hidden" name="id" value="<?php echo $linha['id_local']; ?>">
        <div class="col-md-2">
        ...
        <div class="col-md-6">
            ...
            <input name="nome_local" required class="form-control" id="nome_local"
                value="<?php echo $linha['nome_local'];?>" readonly></input>
        </div>
    </form>
    <br>
    <div class="col-md-12">
